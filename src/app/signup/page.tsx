...
export default function SignupPage() {
  ...
  const isLoginLinkInView = useInView(loginLinkRef, { once: true, margin: "0px" });
  ...
  const formik = useFormik({
    ...
    validationSchema,
    onSubmit: (values) => {
      console.log('Submitting signup with:', { username: values.name, email: values.email });
      signup({
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      googleLogin({
        ...
        onSuccess: () => {
          showToast.success(`Welcome to Theplayfree, ${user.displayName || 'friend'}!`);
        },
        onError: (err: any) => {
          showToast.error(err.message || 'Google signup failed');
        }
      });
    } catch (err: any) {
      if (err.code !== 'auth/popup-closed-by-user') {
        console.error('Google Auth Error:', err);
        showToast.error(err.message || 'Google popup error occurred');
      }
    }
  };
          ...
          <motion.div
            ...
            <form onSubmit={formik.handleSubmit} className="space-y-5">
              ...
              {error && (
                <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg text-sm">
                  ...
                  <p>{error instanceof Error ? error.message : 'An error occurred. Please try again.'}</p>
                </div>
              )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-11 pr-4 py-3 border rounded-lg focus:ring-2 focus:ring-orange-400 focus:border-transparent outline-none transition font-[poppins] ${formik.touched.name && formik.errors.name
                      ? 'border-red-500'
                      : 'border-gray-300'
                      }`}
                ...
                {formik.touched.name && formik.errors.name && (
                  <p className="mt-1 text-xs text-red-600 font-[poppins]">{formik.errors.name}</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-11 pr-12 py-3 border rounded-lg focus:ring-2 focus:ring-orange-400 focus:border-transparent outline-none transition font-[poppins] ${formik.touched.confirmPassword && formik.errors.confirmPassword
                      ? 'border-red-500'
                      : 'border-gray-300'
                      }`}
                    placeholder="••••••••"
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-white text-gray-500 font-[poppins]">Or sign up with</span>
              </div>
            </div>
            ...
            <motion.p
              ref={loginLinkRef}
              initial="hidden"
              animate={isLoginLinkInView ? "visible" : "hidden"}
              variants={fadeUpVariants}
